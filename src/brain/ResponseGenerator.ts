...
export class ResponseGenerator {
  generate(input: string, context: ConversationContext): string {
    ...
    if (context.topic === "science") {
      response = `Yeh bahut interesting question hai! 👀

Brahmand ki shuruaat lagbhag 13.8 billion years pehle Big Bang se hui thi.

Uske baad:
• Particles bane
• Stars aur galaxies bane
• Earth bani (~4.5 billion years)
• Life start hui (~3.5 billion years)

Simple words me:
Universe → Stars → Planets → Life → Humans

✨ Yeh ek slow aur natural evolution process tha.`;
...
Darshanshastra (philosophy) kehta hai ki jeevan ka maksad khud ko explore karna, seekhna aur experience karna hai. Evolution ne humein conscious banaya hai taaki hum is sundar universe ko samajh sakein.`;
    ...
    // 4. TOPIC: CREATOR
      ...
      response = `Main Mamta AI V11 hoon, aur mujhe mere intelligent engineer ne develop kiya hai! 🛠️🧠
...
Mera pipeline multi-brain architectures se powered hai, jisse main instant local learning recall kar sakti hoon aur dynamic planning patterns coordinate kar sakti hoon.`;
    ...
    // 6. DEFAULT GENERAL CHAT
      ...
      if (text.includes("hi") || text.includes("hello") || text.includes("namaste") || text.includes("hey")) {
        response = `Hello aur Namaste! 😊 Aap se baat karke bahut accha laga! Main Mamta AI V11 hoon, aapki cognitive partner.`;
